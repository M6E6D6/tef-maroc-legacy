        ...
        <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
          ...
          <ul className="mt-12 flex flex-col gap-10">
            {trainingCategories.map((cat, i) => (
              <li key={cat.id}>
                <FadeIn delay={i * 0.04}>
                  <article className="rounded-2xl border border-slate-200 bg-slate-50/50 p-6 shadow-sm transition hover:border-[var(--color-gold)]/35 sm:p-8">
                    <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                      <h2 className="font-heading text-xl font-semibold text-[var(--color-navy)] sm:text-2xl">
                        {cat.title}
                      </h2>
                      <p className="shrink-0 text-sm font-medium text-[var(--color-gold)]">
                        {cat.duration}
                      </p>
                    </div>
