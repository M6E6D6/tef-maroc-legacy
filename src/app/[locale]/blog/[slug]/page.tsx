...
import { BLOG_SLUGS, blogImageIndex, getBlogPost, isBlogSlug, type BlogSlug } from "@/data/blog-posts";
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!locales.includes(l as Locale) || !isBlogSlug(slug)) {
    return {};
  }
  ...
  const description = post.meta.description[locale];
...
function heroAltForBlog(locale: Locale, slug: BlogSlug): string {
  const m = translations[locale].mediaAlt;
  const map: Record<BlogSlug, keyof typeof m> = {
    "guest-experience-excellence": "blogGuestExcellence",
    "food-safety-culture": "blogFoodSafety",
  };
  return m[map[slug]];
}
...
export default async function BlogArticlePage({ params }: Props) {
  ...
  const t = dict.blogDetailPage;
  const heroSrc = marketingImages.blog[blogImageIndex[slug]]!;
  const heroAlt = heroAltForBlog(locale, slug);
  const heroAbsolute = heroSrc.startsWith("http") ? heroSrc : absoluteUrl(heroSrc);
  ...
  return (
    <>
      <BlogPostingJsonLd
        ...
        description={post.meta.description[locale]}
      ...
      <div className="bg-white">
        <div className="border-b border-slate-100 bg-gradient-to-b from-slate-50 to-white">
          <div className="mx-auto max-w-6xl px-4 py-10 sm:px-6 sm:py-14 lg:px-8">
            ...
            <h1 className="mt-2 font-heading text-4xl font-bold tracking-tight text-[var(--color-navy)] sm:text-5xl">
              {post.meta.title[locale]}
            </h1>
            <p className="mt-4 max-w-3xl text-lg leading-relaxed text-slate-600">{post.lead[locale]}</p>
          </div>
        </div>
